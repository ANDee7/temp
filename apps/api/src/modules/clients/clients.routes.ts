...
export function createClientsRoutes(clientsService: ClientsService) {
  return async function clientsRoutes(app: FastifyInstance) {
    app.get("/clients", async () => clientsService.list());

    app.post("/clients", async (request, reply) => {
      try {
        const payload = UpsertClientInputSchema.parse(request.body);
        const client = await clientsService.upsert(payload);
